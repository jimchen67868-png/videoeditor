...
class TimelineExporter(private val context: Context) {
        ...
        val transformer = Transformer.Builder(context)
            ...
            .addListener(object : Transformer.Listener {
                override fun onCompleted(
                    ...
                    // Defensive check: onCompleted has occasionally fired
                    // while the output file was still 0 bytes (or otherwise
                    // too small to be a real video), reported intermittently
                    // -- rather than trust the callback alone, verify the
                    // file actually has real content before calling it a
                    // success, so a transient encoder/muxer hiccup surfaces
                    // as a clear "please retry" instead of silently handing
                    // back a broken empty file.
                    val minPlausibleBytes = 10_000L // a real few-second video is always much larger than this
                    if (!outputFile.exists() || outputFile.length() < minPlausibleBytes) {
                        onComplete.onResult(
                            false, null,
                            IllegalStateException("Export produced an unexpectedly small file (${outputFile.length()} bytes) -- please try exporting again")
                        )
                        return
                    }
    ...
    private fun buildVideoSequence(project: Project, settings: ExportSettings): EditedMediaItemSequence {
        ...
        val items = project.clips.mapIndexed { index, clip ->
            ...
            // EXPORT-ONLY (live preview stays disabled -- confirmed unstable
            // there). Previously limited to ONE overlay per clip here as a
            // safety measure while the crash was being diagnosed -- but that
            // crash was in the LIVE PREVIEW specifically, and the actual
            // "text invisible in export" bug turned out to be a text-sizing
            // issue (density-dependent sp needing display context Transformer
            // may not have offline), now fixed. The single-overlay limit was
            // never actually proven necessary for EXPORT -- re-enabling
