    ...
    fun build(context: Context, overlays: List<ImageOverlay>): List<Pair<Int, OverlayEffect>> {
        return overlays.mapNotNull { overlay ->
            ...
            val media3Overlay = object : BitmapOverlay() {
                // Self-calibrating time reference -- see the matching, more
                // detailed comment in TextOverlayEffectFactory for why: don't
                // assume presentationTimeUs resets to ~0 per clip, derive
                // each clip's own "time zero" from the first frame observed.
